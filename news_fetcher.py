...
class GDELTFetcher:
    def __init__(self, gdelt_api_key):
        self.gdelt_api_key = gdelt_api_key
        logger.info("GDELTFetcher initialized")

    def fetch_news(self, keywords):
        try:
            url = f'https://api.gdeltproject.org/api/v2/doc/docsearch?query={"+".join(keywords)}&mode=artlist&format=json&key={self.gdelt_api_key}'
